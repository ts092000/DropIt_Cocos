      ...
      _export("GameController", GameController = (_dec = ccclass('GameController'), _dec2 = property({
        ...
        generateRowBlock() {
          ...
          this.listBlock.map(block => {
            ...
            var index = compBlock == null ? void 0 : compBlock.getIndex();
            compBlock == null ? void 0 : compBlock.setIndex(index.row + 1, index.col);
          });
          var row = new Array(8);
          var rowBlock = this.gameView.generateRowBlock();
          ...
          rowBlock.map(block => {
            ...
            var index = compBlock.getIndex();
            var length = compBlock.getLength();
        ...
        onTouchStartBlock(event) {
          ...
          var row = this.matrixManager.getRow(compBlock.getIndex().row);
          var minIndex = compBlock.getIndex().col - 1 <= 0 ? 0 : compBlock.getIndex().col - 1;
          var maxIndex = compBlock.getIndex().col + compBlock.getLength();
        ...
        onTouchEndBlock(event) {
          ...
          var indexRow = this.currentBlock.getComponent(_crd && Block === void 0 ? (_reportPossibleCrUseOfBlock({
            error: Error()
          }), Block) : Block).getIndex().row;
          var indexCol = this.currentBlock.getComponent(_crd && Block === void 0 ? (_reportPossibleCrUseOfBlock({
            error: Error()
          }), Block) : Block).getIndex().col;
          var length = this.currentBlock.getComponent(_crd && Block === void 0 ? (_reportPossibleCrUseOfBlock({
            error: Error()
        ...
        onTouchMove(event) {
          ...
          var location = this.gameView.getLocation(event);
          var move = location.x - this.startMouseLocation.x;
          var position = new Vec3(this.currentPosition.x + move, this.currentPosition.y);
        ...
        getNextIndexRows() {
          ...
          this.isPause = true;
          var listBlock = this.getListBlock();
          var checkFall = false;
          ...
          var _loop = function _loop() {
            ...
            if (!compBlock) return "continue";
            if (block.active === false) return "continue";
            var indexRow = compBlock.getIndex().row;
            var indexCol = compBlock.getIndex().col;
            var length = compBlock.getLength();
            if (indexRow < 0) return "continue"; //Fall block

            var nextRow = _this.matrixManager.getNextRow(indexRow, indexCol, length);
            ...
            if (checkFall) {
              _this.audioController.playAudio((_crd && AudioType === void 0 ? (_reportPossibleCrUseOfAudioType({
                error: Error()
              }), AudioType) : AudioType).Drop);
            }

            _this.matrixManager.fillValue(indexRow, 0, indexCol, indexCol + length);

            _this.matrixManager.fillValue(nextRow, 1, indexCol, indexCol + length);
          };

          for (var block of listBlock) {
            var _ret = _loop();
          ...
          var listRowFull = this.matrixManager.getListRowFull();
        ...
        clearRow(indexRow) {
          ...
          var listBlock = this.getBlockOfRow(indexRow);
          this.scoreManager.addScore(80 * this.comboRatio[this.combo - 1]);
          this.gameCenter.matchLog({
            score: this.scoreManager.getScore()
          });
          this.combo++;
          listBlock.map(block => {
        ...
        getListBlock() {
          var listBlock = this.blockContainer.children;
...
//# sourceMappingURL=2a350437549c6675b30fe0900a8a92752a1336a2.js.map